...
class GoodsController
{
    ...
    protected function grid()
    {
        return Admin::grid(GoodsShare::class, function (Grid $grid) {
            ...
            $grid->category_id('分类')->editable('select', GoodsCategory::allSelectOptions())->sortable();
            $grid->cover('封面')->image();
            $grid->created_at();
            $grid->updated_at();
            $grid->filter(function($filter){
                // 在这里添加字段过滤器
                $filter->equal('category_id')->select(GoodsCategory::allSelectOptions());
            });
        });
    }
    ...
    protected function form()
    {
        ...
        return Admin::form(GoodsShare::class, function (Form $form) {
           // $form->tag                          =   GoodsTag::tags($form->id);
            ...
            $form->select('category_id','分类')->options(GoodsCategory::allSelectOptions())->default(0);
