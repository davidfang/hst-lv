...
class CreateGoodsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('goods', function (Blueprint $table) {
            $table->increments('id');
            $table->integer('num_iid')->unique()->comment('商品ID');
            ...
            $table->string('item_url')->nullable()->comment('商品地址');
            $table->string('click_url')->nullable()->comment('淘客地址');
            ...
            $table->string('tk_rate')->nullable()->comment('收入比例，举例，取值为20.00，表示比例20.00%');
            $table->string('zk_final_price_wap')->nullable()->comment('无线折扣价，即宝贝在无线上的实际售卖价格。');
            ...
            $table->integer('category')->nullable()->comment('后台一级类目');
            $table->string('coupon_click_url')->nullable()->comment('商品优惠券推广链接');
            $table->string('coupon_end_time')->nullable()->comment('优惠券结束时间');
            $table->string('coupon_info')->nullable()->comment('优惠券面额');
            $table->string('coupon_start_time')->nullable()->comment('优惠券开始时间');
