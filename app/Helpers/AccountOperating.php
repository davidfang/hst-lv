...
class AccountOperating
{
     ...
     * @throws \Throwable
     */
    public static function orderRefund($user_id)
    {
//事务开始
        DB::transaction(function () use ($user_id, &$account) {
            //1. 统计用户上个月25号之前的所有单子 统计返利表中返利金额
            $rebateOrder = RebateOrder::where('unid', $user_id)
                ->where('tk_status', '=', 14)
                ...
                //->select()
                ->first(['unid', \DB::raw('SUM(commission) as commissionSum'), \DB::raw("count(id) as orderCount")]);
            if ($rebateOrder) {
                $amount = $rebateOrder->commissionSum;
                // 2.账户表查询原始记录，锁定
                $account = Account::where('user_id', $user_id)->lockForUpdate()->first();
                if ($account) {
                    //3.订单归帐表写入记录 用户id，返利额，订单总数，类别
                    ...
                    $orderRefund->order = $rebateOrder->orderCount;
                    $orderRefund->type = '自己消费';
                    $orderRefund->save();

                    // 4.写入账户日志表：
                    //             可提现冻结金额减去返利总金额  可提现金额加上返利总金额
                    $accountLog = AccountLog::insertLog($account, $amount, 0, $amount, 0, -$amount, 0, 'OrderRefund', $orderRefund->id, '订单归帐可提现');


                    //5.修改账户表，释放锁定
                    $account->cash_balance = $account->cash_balance + $amount;//可提现余额= 操作前可提现余额 + 发生金额
                    $account->freeze_cash_balance = $account->freeze_cash_balance - $amount;//可提现冻结余额= 操作前可提现冻结余额 - 发生金额
                    $account->save();
                }
            }
        }, 5);
